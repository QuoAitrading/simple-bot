...
def test_strict_blocking_same_device():
    ...
    # Try at different time intervals (up to 39s - should all be blocked)
    for seconds in [10, 20, 30, 39]:
...
def test_no_exceptions_whatsoever():
    ...
    print(f"\n4. Bot running (heartbeat 20s ago)")
    mgr.sessions[license_key] = (device_fp, datetime.now() - timedelta(seconds=20))
    success, msg = mgr.login(license_key, device_fp)
    print(f"   {msg}")
    assert not success, "Should be BLOCKED at 20s"
    
    print(f"\n5. Bot running (heartbeat 39s ago)")
    mgr.sessions[license_key] = (device_fp, datetime.now() - timedelta(seconds=39))
    success, msg = mgr.login(license_key, device_fp)
    print(f"   {msg}")
