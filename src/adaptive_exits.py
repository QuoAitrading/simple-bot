"""\
Adaptive Exit Management - Dynamic Exit Parameters
=================================================
...
class AdaptiveExitManager:
    ...
    def __init__(self, config: Dict, experience_file: str = "data/exit_experience.json"):
        ...
        Args:
            ...
            experience_file: Local file path for saving experiences
        """
        self.config = config
        ...
        logger.info(f"[ADAPTIVE] Exit Manager initialized ({len(self.exit_experiences)} past exits)")
    ...
    def record_exit_outcome(self, regime: str, exit_params: Dict, trade_outcome: Dict, 
        ...
        """
        Record exit outcome for local learning (including scaling decisions).
        ...
        logger.info(f"[EXIT] LEARNED: {regime} | {exit_params['breakeven_threshold_ticks']}t BE, "
    ...
    def update_learned_parameters(self):
        ...
        for regime, outcomes in regime_outcomes.items():
            ...
            if regime not in self.learned_params:
                ...
                logger.info(f"[EXIT] New regime discovered: {regime}, initializing with defaults")
            ...
            if len(wide_stops) >= 3 and len(tight_stops) >= 3:
                ...
                if wide_pnl > tight_pnl + 50:  # Wide stops significantly better
                    ...
                    logger.info(f"[EXIT] {regime}: WIDE stops work better (${wide_pnl:.0f} vs ${tight_pnl:.0f})")
                    ...
                    logger.info(f"[EXIT] {regime}: TIGHT stops work better (${tight_pnl:.0f} vs ${wide_pnl:.0f})")
            ...
            if tight_pnl > standard_pnl and tight_pnl > loose_pnl:
                ...
                logger.info(f"[EXIT] LEARNED: {regime} prefers TIGHT exits (avg P&L: ${tight_pnl:.2f})")
                ...
                logger.info(f"[EXIT] LEARNED: {regime} prefers LOOSE exits (avg P&L: ${loose_pnl:.2f})")
    ...
    def _learn_partial_exit_params(self, regime: str, outcomes: list):
        ...
        if len(early_partials) >= 5:
            ...
            logger.debug(f"[PARTIAL] {regime} Early partials (~2R): ${avg_pnl:.2f} avg, {avg_r:.2f}R")
        ...
        if len(mid_partials) >= 5:
            ...
            logger.debug(f"[PARTIAL] {regime} Mid partials (~2.5R): ${avg_pnl:.2f} avg, {avg_r:.2f}R")
        ...
        if len(late_partials) >= 5:
            ...
            logger.debug(f"[PARTIAL] {regime} Late partials (~3R): ${avg_pnl:.2f} avg, {avg_r:.2f}R")
        ...
        if best_strategy:
            ...
            logger.info(f"[PARTIAL] {regime}: {timing.upper()} partials work best (${best_pnl:.2f}), adjusting to {new_r:.2f}R")
        ...
        if len(aggressive_pct) >= 5 and len(patient_pct) >= 5:
            ...
            if agg_pnl > pat_pnl + 25:  # Aggressive significantly better
                ...
                logger.info(f"[PARTIAL] {regime}: AGGRESSIVE scaling better (${agg_pnl:.2f} vs ${pat_pnl:.2f}), increasing to {new_pct:.0%}")
            elif pat_pnl > agg_pnl + 25:  # Patient significantly better
                new_pct = max(0.30, current_pct * 0.95)
                self.learned_params[regime]['partial_1_pct'] = new_pct
                logger.info(f"[PARTIAL] {regime}: PATIENT scaling better (${pat_pnl:.2f} vs ${agg_pnl:.2f}), decreasing to {new_pct:.0%}")
    ...
    def _learn_sideways_timeout(self, regime: str, outcomes: list):
        ...
        if quick_winners and slow_winners:
            ...
            logger.debug(f"[TIMEOUT] {regime}: Quick (<15min): ${quick_avg:.2f} ({len(quick_winners)} trades), "
        ...
        if len(stalled_losers) >= 5:
            ...
            if stalled_avg_loss < -50 and current_timeout > 8:
                ...
                logger.info(f"[TIMEOUT] {regime}: {len(stalled_losers)} stalled losers (avg ${stalled_avg_loss:.2f}), "
        ...
        # If slow winners exist and profitable, loosen timeout
            ...
            if slow_avg > 100 and current_timeout < 25:
                ...
                self.learned_params[regime]['sideways_timeout_minutes'] = new_timeout
                logger.info(f"[TIMEOUT] {regime}: {len(slow_winners)} slow winners (avg ${slow_avg:.2f}), "
    ...
    def _learn_from_market_patterns(self, regime: str, outcomes: list):
        ...
        if len(high_rsi_exits) >= 5:
            ...
            logger.info(f"[EXIT PATTERN] {regime} @ RSI>65: ${avg_pnl:.2f} avg | "
        ...
        if len(high_vol_exits) >= 5:
            ...
            logger.info(f"[EXIT PATTERN] {regime} @ High Volume: ${avg_pnl:.2f} avg | "
        ...
        if len(afternoon_exits) >= 5:
            ...
            logger.info(f"[EXIT PATTERN] {regime} @ Afternoon (13-15h): ${avg_pnl:.2f} avg | "
    ...
    def _learn_runner_hold_criteria(self, regime: str, outcomes: list):
        ...
        if best_group and 'runner_hold_criteria' in self.learned_params[regime]:
            ...
            if best_group == 'high':
                ...
                logger.info(f"[RUNNER] {regime}: HIGH runners (6R+) most profitable, targeting {new_min_r:.1f}R")
            elif best_group == 'med':
                new_min_r = min(8.0, current_min_r * 0.9 + 4.5 * 0.1)
                logger.info(f"[RUNNER] {regime}: MED runners (4-6R) most profitable, targeting {new_min_r:.1f}R")
            else:
                new_min_r = max(3.0, current_min_r * 0.9 + 3.5 * 0.1)
                logger.info(f"[RUNNER] {regime}: LOW runners (3-4R) most profitable, targeting {new_min_r:.1f}R")
        ...
        if len(long_holds) >= 5 and len(short_holds) >= 5:
            ...
            if 'runner_hold_criteria' in self.learned_params[regime]:
                ...
                if long_avg > short_avg + 25:  # Long holds significantly better
                    ...
                    logger.info(f"[RUNNER] {regime}: Long holds (30+ min) better (${long_avg:.2f} vs ${short_avg:.2f}), "
                    ...
                    self.learned_params[regime]['runner_hold_criteria']['min_duration_minutes'] = new_min_dur
                    logger.info(f"[RUNNER] {regime}: Short holds (<20 min) better (${short_avg:.2f} vs ${long_avg:.2f}), "
        ...
        if len(drawdown_trades) >= 5 and 'runner_hold_criteria' in self.learned_params[regime]:
            ...
            if avg_loss < -50:
                ...
                logger.info(f"[RUNNER] {regime}: {len(drawdown_trades)} high-drawdown losers (avg ${avg_loss:.2f}), "
                           f"tightening max drawdown to {new_dd:.2%}")

    
    def load_experiences(self):
        """Load past exit experiences from local file."""
    ...
    def get_stop_multiplier(self, regime: str, recent_exits: list = None) -> float:
        ...
        logger.info(f"[EXIT] Using {regime} stop multiplier: {stop_mult:.1f}x ATR")
        return stop_mult
    ...
    def should_hold_runner(self, position: dict, current_market: dict, r_multiple: float) -> bool:
        ...
        if len(big_winners) >= 15:
            ...
            if r_multiple < avg_big_r * 0.8:
                logger.info(f"[EXIT] Hold runner - {len(big_winners)} similar averaged {avg_big_r:.1f}R")
                return True
    ...
    def save_experiences(self):
        """
        Save exit experiences to local file.
        """

        try:
            os.makedirs(os.path.dirname(self.experience_file) or '.', exist_ok=True)
            ...
            logger.info(f"[LOCAL] Saved {len(self.exit_experiences)} exit experiences")
        except Exception as e:
...
def _find_similar_exit_experiences(market_state: Dict, regime: str, all_experiences: list, min_similarity: float = 0.60) -> list:
    ...
    Uses 9-feature similarity scoring.
...
def get_adaptive_exit_params(bars: list, position: Dict, current_price: float, 
    ...
    # Cloud exit recommendations removed; use local learned parameters below.
    ...
    if adaptive_manager and hasattr(adaptive_manager, 'learned_params'):
        ...
        if exploration_rate > 0 and random.random() < exploration_rate:
            ...
            logger.debug(f"[LEARNED] {market_regime}: BE={breakeven_threshold_multiplier:.2f}x, Trail={trailing_distance_multiplier:.2f}x")
    else:
