...
# Using eventlet mode for Azure App Service with gunicorn eventlet workers
...
# SYMBOL GROUPS - Micro contracts map to their full-size equivalents
# Key = symbol user might trade, Value = base symbol for instrument lookup
SYMBOL_GROUPS = {
...
def get_base_symbol(symbol: str) -> str:
    """Get the base symbol for instrument lookup. MES -> ES, MNQ -> NQ, etc."""
...
def hello():
    ...
    return jsonify({
        "status": "success",
        "message": "✅ QuoTrading Trade Copier API",
        "endpoints": [
            "POST /copier/register - Register as follower",
            "POST /copier/heartbeat - Keep connection alive",
            "POST /copier/broadcast - Broadcast signal to followers",
            "GET /copier/poll - Poll for new signals",
            "GET /copier/status - Get copier system status",
            "GET /api/hello - Health check"
        ],
        "database_configured": bool(DB_PASSWORD),
        "note": "Trade copier relay server for master/follower signal distribution"
    }), 200
...
def handle_subscribe(data):
    """
    Client subscribes to updates for specific symbols.
    ...
    emit('subscribed', {
        'message': f'Subscribed to updates for: {", ".join(set(rooms_joined))}',
